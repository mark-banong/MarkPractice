...
<div id="sign-in-container" class="container-fluid login-container <?php echo (!$_GET['login']) ? 'hidden' : ''; ?>">
	<form name="login-form" id="login-form" action="<?php get_site_url(); ?>/wp-admin/admin-post.php" method="POST">
	<input type="hidden" value="validate_user" name="action">
		...
		<div class="row">
			<div class="col-md-12">
				<?php if ($_GET['admin'] || isset($_GET['admin'])) { ?> 
					<label class="center color-white" >User Name</label>
					<input id="user_login" type="text" size="20" value="" name="log" placeholder="xxxxxxxxxx" />
				<?php } else { ?>
				...
				<?php } ?>
				<label class="center color-white" >Password</label>
				<input id="user_pass" type="password" size="20" value="" name="pwd" placeholder="xxxxxxxxxx"/>
			</div>
		</div>
...
<div id="register-container" class="container-fluid login-container <?php echo (!$_GET['register']) ? 'hidden' : ''; ?>">
	<form id="register-form" action="<?php get_site_url(); ?>/wp-admin/admin-post.php" method="POST">
		<input type="hidden" name="action" value="send_code" />
		<div class="row">
			<div class="col-md-12">
				<p class="error center <?php echo ($_GET['register'] === 'false') ? '' : 'hidden'; ?>">Phone number not found</p>
				<p class="error center <?php echo ($_GET['register'] === 'returnee') ? '' : 'hidden'; ?>">This account is already registered.</p>				
			</div>
		</div>
...
<div id="verify-container" class="container-fluid login-container <?php echo ($_GET['verify']) ? '' : 'hidden'; ?>">
	<form id="verify-form"  action="<?php get_site_url(); ?>/wp-admin/admin-post.php" method="POST">
		...
		<div class="row">
			<div class="col-md-12">
				<label class="center color-white" >Verification Code</label>
				<input id="user_verification_code" type="text" value="" name="verificationCode" placeholder="Ex.: 31548" />
			</div>
		</div>
	</form>
	...
	<form id="resend-form"  action="<?php get_site_url(); ?>/wp-admin/admin-post.php" method="POST">
		<input type="hidden" name="action" value="resend_code" />

		<div class="row">
			<div class="col-md-12">
				<button class="color-white" type="submit" form="resend-form">Resend Verification Code</button>			
			</div>
		</div>
	</form>
</div>
...
<div id="registration-container" class="container-fluid login-container <?php echo ($_GET['registration']) ? '' : 'hidden'; ?>">
	<form id="registration-form"  action="<?php //get_site_url(); ?>/wp-admin/admin-post.php" method="POST">	
